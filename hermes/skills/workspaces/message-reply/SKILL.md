...
## Engine (reuse the sibling CLIs; never open their DBs/files)
Siblings share the cluster dir, so from this skill:
...
## 0. Parse the request
Separate three things from what the user sent:
...
## 2. Classify the scope (Projects / Personal / unrelated)
Decide what the **content** is about — person-first, then reconcile with the text:
...
## 3. Gather context for that scope
Pull just enough to interpret and reply well — summarize, never dump:
...
Answer "what does this mean?": in the gathered context, explain what the sender is saying and
what (if anything) they're asking for or expect back. Flag uncertainty rather than guessing.
...
## Safety gates (People/Personal data sensitive; project calibration semi-private)
