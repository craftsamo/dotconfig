...
## Families served by hands (checked FIRST — [Plan](../plan-creator/SKILL.md) / [Build](../build-creator/SKILL.md))
...
| HTML/CSS/JS motion outside the served create-motion/create-tour/create-ad/create-explainer-video scopes: overlays on existing footage, captioned narration, audio-reactive visuals, pieces over 60 seconds | `creator-html-motion` | HyperFrames source project + MP4/WebM; supporting generation is separately budgeted; authored launch/promo/brand motion goes to create-motion first |
...
7. Bounded UI task walkthroughs select create-tour, and authored motion
   design (launch/promo/brand/kinetic type, 3..60s) selects create-motion,
   before the legacy table; neither retires creator-html-motion or its 1:1
   mapping. Creator does not author such a piece itself when create-motion
   fits.
   Route by authorship method as well as container. A model-generated MP4 is
    ...
    through generate-clip or guarantee exact music/lettering. Remaining seekable HTML motion outside those
   served scopes is `creator-html-motion`; p5.js canvas/WebGL work is
