...
`scripts/lint.py` が返す finding はカテゴリごとに性格が違います。
この表は「そのカテゴリが何を疑っているか」と「直すならどの方向か」を引くための一覧です。
2026-07 のコーパス校正（人間103文書 + AI 81文書の実測）で複数の検出器の前提が実データと逆だったことがわかり、方向の反転、削除、格下げを行いました。
その経緯も各行に残しています。
...
| `antithesis_repetition` | 「〜ではなく」型の対比が3回以上。2026-07 の再校正で、出現ごとの一律 critical をやめ「検出数/総文数」の比率で severity を3段階化した | severity=info は参考情報（人間の修辞技法と区別のつかない薄い頻度）。warn/critical は比率が高く、実測で真陽性の多い帯。severity にかかわらず全部を直す必要はなく、最も重要な対比だけを残して他は素直な肯定文や具体例に変える |
...
EXPERIMENTAL 系（`--experimental` を付けない限り出力されない）は、コーパスでの定量校正がまだか、human/ai ともほぼ発火しないと判定された検出器です。
校正が進むまで、判断材料としての信頼度は低いものとして扱います。

`--genre essay|tech|business` を指定すると、ジャンル別にコーパス校正した閾値プロファイルへ切り替わります（未指定時は共通の保守的閾値です）。
ジャンルごとの判断の重みづけは `genre-notes.md` を参照します。

すべての finding を機械的に潰す必要はありません。
検出は疑いの提示にすぎず、文脈上そのままで自然な箇所も混じっています。
判断に迷ったら、その一文を声に出して読み、実際に人が話すときの言い回しに近いかどうかで決めます。
