...
# ]
# ///
"""terms.py — 専門用語候補を初出順に抽出するスクリプト。

目的:
    カタカナ複合語、ASCII 英略語、固有名詞らしき語を列挙し、初出位置と説明の手掛かりを
    判断材料として提示する。有用性や説明済みかどうかの判定は行わない。
...
共有基盤:
    Sudachi tokenizer、Markdown 構造マスク、行番号処理、ファイル読み込みは textcore.py
    を使う。lint.py と outline.py も同じ基盤を共有する。
"""
...
# --- terms: カタカナ複合語・ASCII 英略語・固有名詞候補を列挙する ---
# 有用性や初出での説明済みかどうかは判定せず、has_gloss_hint は機械的な手掛かりにとどめる。
