...
# ]
# ///
"""outline.py — 文書のスケルトンと見出し統計を決定的に抽出するスクリプト。

目的:
    見出し、段落の先頭文、箇条書きブロックと、見出しの構造統計を判断材料として提示する。
    良し悪しや AI 臭さの判定は行わない。
...
共有基盤:
    Sudachi tokenizer、Markdown コメント処理、見出し解析、ファイル読み込みは
    textcore.py を使う。lint.py と terms.py も同じ基盤を共有する。
"""
...
# --- outline: 見出し・段落の先頭文・箇条書きブロックを抽出する ---
# 文書構造を決定的に抽出するだけで、良し悪しの判断はしない。
#
...
# --- heading_stats: 見出しの構造統計を判断材料として提示する ---
# Finding や severity は生成せず、本数・長さ・体言止め・品詞パターンなどを集計する。
#
